...
void DialogOption::touchHandle(Vec2 pt)
{
    ...
    {
        spMusic->setScale(0.9);
    }
    
    if(spSound->getBoundingBox().containsPoint(pt))
    {
        spSound->setScale(0.9);
    }
    
    if(spReset->getBoundingBox().containsPoint(pt))
    {
        spReset->setScale(0.9);
    }
    ...
    else
    {
        ...
        else if(spReset->getBoundingBox().containsPoint(pt))
        {
            ...
            if(LanguageType::KOREAN == Application::getInstance()->getCurrentLanguage())
                label = LabelCreater::createLabel("게임 데이타가 초기화 됩니다", "JejuHallasan.ttf", 26);
            else
                label = LabelCreater::createLabel("Data initialization in progress", "JejuHallasan.ttf", 22);
