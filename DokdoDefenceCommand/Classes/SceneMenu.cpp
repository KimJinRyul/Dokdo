    ...
    if (keycode == EventKeyboard::KeyCode::KEY_BACK) {
        ...
        if (bBack == false) {
            ...
            if (Application::getInstance()->getCurrentLanguage() == LanguageType::KOREAN) {
                label = LabelCreator::createLabel("Back 버튼을 한 번 더 누르면 종료합니다.", "JejuHallasan.ttf", 25);
            } else {
                label = LabelCreator::createLabel("Press back one more time to quit.", "JejuHallasan.ttf", 25);
            }
